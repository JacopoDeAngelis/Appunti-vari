...
\chapter{Servizi REST}
\section{Usare gli XML}
Utile per servizi che non sono passati a XML, esiste l'XmlSlurper che si occupa di leggere e interpretare le informazioni di un documento XML e salvarle, ad esempio:
\begin{lstlisting}[language = groovy]
...
// il file contiene lo stesso XML scritto sopra
def sports = new XmlSlurper().parse('[indirizzo di un file coi dati]')

println sports
println sports.sport[2].name
\end{lstlisting}

\section{Usare i Json}
Fondamentalmente funziona allo stesso modo ma con i JSON:
\begin{lstlisting}[language = groovy]
...
json = slurper.parse(new File('[indirizzo di un file coi dati]'))
...
\section{Databases}
\href{https://docs.groovy-lang.org/latest/html/groovy-jdk/groovy/sql/package-frame.html}{Documentazione}
